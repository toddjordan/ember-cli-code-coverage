...
function fixFilePaths(coverageData, fileLookup, configPath) {
  let found = getConfig(configPath).coveredAddons.find((item) =>  coverageData.path.startsWith(item));
  if (found) {
...
function writeCoverage(coverage, fileLookup, root, configPath, map) {
  ...
  Object.keys(fileLookup).forEach(filename => {
    ...
    map.addFileCoverage(fixFilePaths(fileCoverage, fileLookup, configPath));
  });
}
...
function coverageHandler(map, options, req, res) {
  writeCoverage(req.body, options.fileLookup, options.root, options.configPath, map);
